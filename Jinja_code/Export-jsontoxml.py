...
def create_Dynamic_xml_dict(Object_dict, Output_XML_file, XML_template):
    ...
    print sub_directory
    # Create environmental variable in the location of our template (/template/jsontoxmltemplate_1.xml)
    env = jinja2.Environment(loader=jinja2.FileSystemLoader(["./" + Location_list[0]]))
    Our_template = env.get_template(Location_list[1]) # Get our template jsontoxmltemplate.xml from env

    for element in Object_dict: # Object_dict is a dictionary created from json data 
		if element == "data":
			user_entry = Object_dict[element] # user_entry is a list jsontoxmltemplate.xml 
			result = Our_template.render(user_entry=user_entry)
    
    # Attach Timestamp information to show when the output xml file is modified 
    ts = time.time() 
    current_time = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%y %H:%M:%S')
    print "Current Time:", current_time
    Modified_Output_XML_file_list = Output_XML_file.split('.')
    Modified_Output_XML_file_list[0] = Modified_Output_XML_file_list[0] + str(current_time) + "."
    Modified_Output_XML_file = ''.join(Modified_Output_XML_file_list)
